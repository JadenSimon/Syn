...
export type PathArgs<T extends string> = T extends `${infer P}{${infer U}}${infer S}` ? [...PathArgs<P>, string, ...PathArgs<S>] : [];
export type PathArgsWithBody<T extends string, U> = [...PathArgs<T>, ...(U extends undefined ? [body?: U] : unknown extends U ? [body?: any] : [body: U])];
