...
pub fn ComptimeStringMapWithKeyType(comptime KeyType: type, comptime V: type, comptime kvs_list: anytype) type {
    ...
    const precomputed = comptime blk: {
        ...
        const lenAsc = (struct {
            fn lenAsc(context: void, a: KV, b: KV) bool {
                ...
                // if (KeyType == u8) {
                //     return strings.orderComptime(a.key, b.key) == -1;
                // } else {
                //     return std.mem.order(KeyType, a.key, b.key) == .lt;
                // }
            }
    ...
    return struct {
        ...
        inline fn binarySearch(str: []const KeyType, comptime start: usize, comptime end: usize) ?V {
            if (start == end) return null;

            const middle = comptime ((end - start) / 2) + start;
            const c = strings.orderComptime(str, kvs[middle].key);
            if (c == 0) {
                return kvs[middle].value;
            } else if (c == -1) {
                return binarySearch(str, start, middle);
            } else {
                return binarySearch(str, middle + 1, end);
            }
        }
        ...
        pub fn getWithLength(str: []const KeyType, comptime len: usize) ?V {
            ...
            // if (comptime KeyType == u8 and (end - len_indexes[len]) > 2) {
            //     return binarySearch(str, len_indexes[len], end);
            // }
